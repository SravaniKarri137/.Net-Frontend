...
const Marketingjobs = () => {
  ...
  const fetchJobPostings = async () => {
    try {
      const department = encodeURIComponent("Marketing & Sales");
...
<div className="text-center mb-5">
        <motion.h2
          className="fw-bold"
          style={{ color: "#e83e8c", fontFamily: "Segoe UI, sans-serif" }}
          initial={{ scale: 0.8 }}
          animate={{ scale: 1 }}
          transition={{ delay: 0.2 }}
        >
          📣 Marketing & Sales Job Opportunities
        </motion.h2>
        <p className="text-muted">Drive growth, connect with audiences, and close the next big deal!</p>
      </div>
